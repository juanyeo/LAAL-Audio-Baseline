...
class LoRA_ViT_timm(nn.Module):
    def __init__(self, vit_model: timm_ViT, r: int, num_classes: int = 0, lora_layer=None):
        ...
        if lora_layer:
            ...
            self.lora_layer = list(range(len(vit_model.v.blocks)))
        ...
        # Here, we do the surgery
        for t_layer_i, blk in enumerate(vit_model.v.blocks):
            # If we only want few lora layer instead of all
            if t_layer_i not in self.lora_layer:
                continue
        ...
        if num_classes > 0:
            self.lora_vit.v.reset_classifier(num_classes=num_classes)
    ...
    def save_fc_parameters(self, filename: str) -> None:
        ...
        """
        assert filename.endswith(".safetensors")
        _in = self.lora_vit.v.head.in_features
        _out = self.lora_vit.v.head.out_features
        fc_tensors = {f"fc_{_in}in_{_out}out": self.lora_vit.v.head.weight}
        save_file(fc_tensors, filename)
    ...
    def load_fc_parameters(self, filename: str) -> None:
        ...
        with safe_open(filename, framework="pt") as f:
            ...
            try:
                saved_tensor = f.get_tensor(saved_key)
                self.lora_vit.v.head.weight = Parameter(saved_tensor)
            except ValueError:
                print("this fc weight is not for this model")
