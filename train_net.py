...
@torch.no_grad()
def eval_epoch_esc(val_loader, model, val_meter, cur_epoch, cfg, wandb_log=False):
    """
    Evaluate the model on the val set.
    Args:
        val_loader (loader): data loader to provide validation data.
...
def train(cfg):
    ...
    result = np.zeros([cfg.SOLVER.MAX_EPOCH, 6])
    for cur_epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCH):
        # Shuffle the dataset.
        loader.shuffle_dataset(train_loader, cur_epoch)

        # Train for one epoch.
        if cfg.TRAIN.DATASET.upper() == "ESC50":
            train_epoch_esc(
        ...
        if is_eval_epoch:
            if cfg.TRAIN.DATASET.upper() == "ESC50":
                is_best_epoch, scores_dict = eval_epoch_esc(val_loader, model, val_meter, cur_epoch, cfg, wandb_log)
                result[cur_epoch,:] = [scores_dict["mAP"], cur_epoch, scores_dict["top1_err"], scores_dict["top5_err"], scores_dict["mAUC"], scores_dict["mPCA"]]
                np.savetxt(cfg.ESC.RESULT_DIR + "/result.csv", result, delimiter=",")
            else:
