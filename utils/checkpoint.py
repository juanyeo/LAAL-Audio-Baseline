...
def load_checkpoint(
    ...
    Args:
        ...
        deflation (bool): if True, deflate the weights from the checkpoint (3D->2D).
    ...
    if convert_from_caffe2:
        ...
        for key in caffe2_checkpoint["blobs"].keys():
            ...
            if converted_key in ms.state_dict():
                if deflation:
                    if len(caffe2_checkpoint["blobs"][key].shape) == 5:
                        caffe2_checkpoint["blobs"][key] = np.mean(caffe2_checkpoint["blobs"][key], axis=4)
                        if caffe2_checkpoint["blobs"][key].shape[1] == 3:
                            caffe2_checkpoint["blobs"][key] = np.mean(caffe2_checkpoint["blobs"][key], axis=1, keepdims=True)
...
def load_train_checkpoint(cfg, model, optimizer):
    ...
    if cfg.TRAIN.AUTO_RESUME and has_checkpoint(cfg.OUTPUT_DIR):
        ...
        checkpoint_epoch = load_checkpoint(
            ...
            deflation=(cfg.MODEL.MODEL_NAME == "SlowFastAudio"),
            convert_from_caffe2=cfg.TRAIN.CHECKPOINT_TYPE == "caffe2",
